...
mod wallet_properties {
	...
	pub async fn run<A: BarkPersister, B: BarkPersister>(a: &A, b: &B) {
		...
		test_set_mailbox_pubkey(a, b).await;
	}
	...
	async fn test_set_mailbox_pubkey<A: BarkPersister, B: BarkPersister>(a: &A, b: &B) {
		let pk = test_pubkey();

		let ra = a.set_mailbox_pubkey(pk).await;
		let rb = b.set_mailbox_pubkey(pk).await;
		assert_eq!(ra.is_ok(), rb.is_ok(), "set_mailbox_pubkey: ok/err mismatch");

		let ra = a.read_properties().await.expect("a: read_properties after set_mailbox_pubkey");
		let rb = b.read_properties().await.expect("b: read_properties after set_mailbox_pubkey");
		assert_eq!(ra, rb, "read_properties after set_mailbox_pubkey mismatch");
	}
}
