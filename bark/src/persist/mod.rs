...
pub trait BarkPersister: Send + Sync + 'static {
	...
	///
	/// This is used to store the mailbox pubkey for existing wallets that were
	...
	///
	/// Parameters:
	/// - mailbox_pubkey: The server's mailbox public key to store.
	///
	/// Errors:
	/// - Returns an error if the update fails.
	async fn set_mailbox_pubkey(&self, mailbox_pubkey: PublicKey) -> anyhow::Result<()>;
