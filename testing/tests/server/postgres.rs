...
async fn vtxo_mailbox() {
	...
	let cp1 = db.store_vtxos_in_mailbox(mailbox_id.clone(), &[vtxo1.clone()]).await.unwrap()
	...
	let (returned_cp, vtxos) = &batches[0];
	assert_eq!(*returned_cp, cp1);
	assert_eq!(vtxos.len(), 1);
	assert_eq!(vtxos[0].id(), vtxo1.id());

	// Store vtxo2 in mailbox (gets checkpoint 2, ≥ cp1)
	let cp2 = db.store_vtxos_in_mailbox(mailbox_id.clone(), &[vtxo2.clone()]).await.unwrap()
		.expect("should return a checkpoint");
	...
	assert_eq!(batches.len(), 1);
	let (returned_cp2, vtxos2) = &batches[0];
...
async fn store_vtxos_in_mailbox_empty() {
	...
	let result = db.store_vtxos_in_mailbox(mailbox_id, &[]).await.unwrap();
...
async fn round_participation() {
	...
	db.try_store_round_participation(unlock_preimage, &[vtxo.id()], std::iter::once(&output))
		.await.unwrap();
...
async fn round_participation_same_vtxo_multiple_pending() {
	...
	db.try_store_round_participation(preimage1, &[vtxo.id()], std::iter::once(&output)).await.unwrap();
	...
	db.try_store_round_participation(preimage2, &[vtxo.id()], std::iter::once(&output)).await.unwrap();
